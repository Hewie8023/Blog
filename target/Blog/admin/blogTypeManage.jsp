...
<script type="text/javascript">
	...
	function openBlogTypeModifyDialog(){
		...
		if(selectedRows.length!=1){
			$.messager.alert("系统提示","请选择一个要修改的博客类型！");
			return;
		}
		var row = selectedRows[0];
		$("#dlg").dialog("open").dialog("setTitle","修改博客类别信息");
		$("#fm").form("load",row);
		url="${pageContext.request.contextPath}/admin/blogType/save.do?blogTypeId="+row.blogTypeId;
	}
				...
				$.post("${pageContext.request.contextPath}/admin/blogType/delete.do",{ids:ids},function(result){
					if(result.success){
						if(result.errorInfo.length > 0){
							for(var i = 0; i < result.errorInfo.length; i++){
								$.messager.alert("系统提示",result.errorInfo[i]);
							}
						}else{
...
<div id="dlg" class="easyui-dialog" style="width: 550px;height: 180px; padding: 10px 20px;" closed="true" buttons="#dlg-buttons">
	<form id="fm" method="post">
		<table cellspacing="8px">
			...
			<tr>
				<td>博客类别排序号：</td>
				<td>
					<input type="text" id="orderNo" name="orderNo" class="easyui-numberbox" required="true" style="width: 60px;"/>&nbsp;(类别按照排序序号从小到大排序)
				</td>
			</tr>
		</table>
	</form>
